...
static void heapSort(_RandomAccessIterator _first,
...
static void bubblesort(
    _RandomAccessIterator _first,
    _RandomAccessIterator
        _last) {  // Bubble largest element in a[0:_size-1] to _last.
...
static void doublebubblesort(
    ...
    for (; _first < --_last && swapped; ++_first) {
        ...
        for (i = _first; i < _last; ++i)
            if (*i > *(i + 1)) iter_swap(i, i + 1), swapped = true;
        for (i = _last; i > _first; --i)
            if (*(i - 1) > *i) iter_swap(i, i - 1), swapped = true;
    }
}
//[)
template <typename _RandomAccessIterator>
static void selectsort(_RandomAccessIterator _first,
                       _RandomAccessIterator _last) {
    _RandomAccessIterator i;
...
//[)
template <typename _RandomAccessIterator>
static void doubleselectsort(_RandomAccessIterator _first,
                             _RandomAccessIterator _last) {
...
//[)
template <typename _RandomAccessIterator>
static void inline insertsort_bin(
    _RandomAccessIterator _first,
    _RandomAccessIterator _last) {  // [_first, _last)
    if (_first < _last)
...
template <
    ...
    typename _Tp = typename iterator_traits<_RandomAccessIterator>::value_type>
static void inline insertsort(_RandomAccessIterator _first,
                              _RandomAccessIterator _last) {  // [_first, _last)
    if (_first < _last) {
        ...
        for (i = _first + 1; i < _last; ++i) {
            ...
            for (j = i; _first<j &&*(j - 1)> _val; --j) *j = *(j - 1);
            *j = _val;
        }
    }
}
...
static void shellsort(_RandomAccessIterator _first, _RandomAccessIterator _last,
    ...
    for (c = 0; _size <= __Sedgewick[c]; ++c)
        ;
    for (increment = __Sedgewick[c]; increment > 0;
         increment = __Sedgewick[++c])
        for (i = _first + increment; i < _last; ++i) {
            _Tp _tmp = *i;
            for (j = i; _first + increment <= j && *(j - increment) > _tmp;
                 j -= increment)
                *j = *(j - increment);
            *j = _tmp;
        }
...
template <
    typename _RandomAccessIterator,
    typename _Tp = typename iterator_traits<_RandomAccessIterator>::value_type>
static inline void __merge(_RandomAccessIterator _first,
    ...
    while (i <= _mid) merged[_size++] = *(i++);
    while (j <= _last) merged[_size++] = *(j++);
    copy(merged, merged + _size, _first);
}

//[]
template <typename _RandomAccessIterator>
static void __mergesort(_RandomAccessIterator _first,
                        _RandomAccessIterator _last) {
    if (_first < _last) {
...
template <typename _RandomAccessIterator>
static void __mergesort_non_recur(_RandomAccessIterator _first,
                                  _RandomAccessIterator _last) {
    int len = 1, _size = _last - _first;
    for (; len < _size; len <<= 1)
        for (_RandomAccessIterator lo = _first; lo + len < _last;
             lo += (len << 1)) {
...
//[)
template <typename _RandomAccessIterator>
static void inline mergeSort(_RandomAccessIterator _first,
                             _RandomAccessIterator _last) {
...
// []
template <typename _RandomAccessIterator>
static inline void __median3(_RandomAccessIterator _first,
                             _RandomAccessIterator _last) {
    _RandomAccessIterator _mid = _first + (_last - _first) / 2;
    if (*_first > *_mid) iter_swap(_first, _mid);
    if (*_first > *_last) iter_swap(_first, _last);
    if (*_mid > *_last) iter_swap(_mid, _last);
...
template <
    typename _RandomAccessIterator,
    typename _Tp = typename iterator_traits<_RandomAccessIterator>::value_type>
static _RandomAccessIterator __partition(_RandomAccessIterator _first,
                                         _RandomAccessIterator _last) {
    __median3(_first, _last);
    _Tp pivot = *_last;
    _RandomAccessIterator i = _first - 1, j = _first;
    for (; j < _last; ++j)
        if (*j < pivot) iter_swap(++i, j);
    iter_swap(i + 1, _last);
    return i + 1;
}
...
#define CUTOFF 10
template <typename _RandomAccessIterator>
static void __quicksort(_RandomAccessIterator _first,
                        _RandomAccessIterator _last) {
    if (_first < _last) {
...
template <typename _RandomAccessIterator>
static void __quicksort_nonrecur(_RandomAccessIterator _first,
                                 _RandomAccessIterator _last) {
    ...
    if (_first < _last) {
        ...
        while (_s.size()) {
            ...
            if (_tmp.first < p) _s.push(make_pair(_tmp.first, p - 1));
            if (p < _tmp.second) _s.push(make_pair(p + 1, _tmp.second));
        }
    }
}
//[)
template <typename _RandomAccessIterator>
static void inline quicksort(_RandomAccessIterator _first,
                             _RandomAccessIterator _last) {
...
//[]
template <typename _RandomAccessIterator>
static _RandomAccessIterator __kth_element(_RandomAccessIterator _first,
                                           _RandomAccessIterator _last, int k) {
    _RandomAccessIterator p = __partition(_first, _last);
    int rank = p - _first;
    if (rank == k)
        return p;
...
//[)
template <typename _RandomAccessIterator>
static inline _RandomAccessIterator kth_element(_RandomAccessIterator _first,
                                                _RandomAccessIterator _last,
                                                int k) {
...
static void __partial_k_sort(_Tp *_first, _Tp *_last, int k) {
    ...
    _Tp *walk = _last;
    if (k > _last - _first) return;
    makeheap(_first, _last);
    for (; _last - walk < k; walk--) popheap(_first, walk);
}
...
O(N + KlogN)
2:Keep a min heap of k elements. Compare a new element with the root and,
DeletMin and Insert the new element (if the new one is larger.) O(K + NlogK)
...
void radix_sort(_RandomAccessIterator _first, _RandomAccessIterator _last) {
    ...
    for (int dig = 0; dig < _max_digit; dig++) {
        ...
        for (int i = 0; i < _radix; ++i) {
            _container.insert(_container.end(), _bucket[i].begin(),
                              _bucket[i].end());
            _bucket[i].clear();
        }
    }

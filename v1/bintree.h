...
class bintree {
   protected:
    ...
    inline binode_ptr<_Tp> __newbinode(const _Tp &x,
                                       binode_ptr<_Tp> p = nullptr,
                                       binode_ptr<_Tp> l = nullptr,
                                       binode_ptr<_Tp> r = nullptr,
                                       RBColor cl = BLK) {
    ...
    static inline void __updatedepth(binode_ptr<_Tp> &opnv) {
        opnv->depth = _depth(opnv->parent) + 1;
    }
    static inline void __updateheight(binode_ptr<_Tp> &opnv) {
        opnv->height = max(_height(opnv->lc), _height(opnv->rc)) + 1;
    }
    ...
    inline void __update_status() { __update_member(_root, nullptr); }
    inline void __update_root(binode_ptr<_Tp> opnv) { _root = opnv; }
    binode_ptr<_Tp> __build_pi(int opnv, int lo, int hi, binode_ptr<_Tp> p) {
        ...
        node->parent = p;
        __updatedepth(node);
        while (i < hi && inorder[i] != preorder[opnv]) i++;
        node->lc = __build_pi(opnv + 1, lo, i - 1, node);
        node->rc = __build_pi(opnv + 1 + i - lo, i + 1, hi, node);
        __updateheight(node);
        return node;
    }
    void __print_horizon(binode_ptr<_Tp> opnv, int root_x, int root_y,
                         int interval) {
        if (!opnv) return;
        string tp;
        if (IS_CHAR<_Tp>())
        ...
        if (_isRBtree) tp += opnv->color == RED ? "🔴" : "⚫️";
        while (tp.size() < _unitsize) tp.push_back(' ');
        this->__display_buffer[root_x][root_y] = tp;
        if (opnv->lc)
            this->__display_buffer[root_x + 1][root_y - (interval + 1) / 2] =
                _direct2l;
        if (opnv->rc)
            this->__display_buffer[root_x + 1][root_y + (interval + 1) / 2] =
                _direct2r;
        __print_horizon(opnv->lc, root_x + 2, root_y - interval,
                        (interval >> 1));
        __print_horizon(opnv->rc, root_x + 2, root_y + interval,
                        (interval >> 1));
    }
    binode_ptr<_Tp> __build_ip(int opnv, int lo, int hi, binode_ptr<_Tp> p) {
        ...
        node->parent = p;
        __updatedepth(node);
        while (i < hi && inorder[i] != postorder[opnv]) i++;
        node->lc = __build_ip(opnv - 1 + i - hi, lo, i - 1, node);
        node->rc = __build_ip(opnv - 1, i + 1, hi, node);
        __updateheight(node);
        return node;
    }
    binode_ptr<_Tp> __build_pp(int leftOfpre, int rightOfpre, int leftOfpost,
                               int rightOfpost) {
        if (leftOfpre > rightOfpre || leftOfpost > rightOfpost) return nullptr;
        binode_ptr<_Tp> opnv = this->__newbinode(preorder[leftOfpre]);
        if (leftOfpre == rightOfpre) return opnv;
        ...
        {
            opnv->lc = __build_pp(leftOfpre + 1, leftOfpre + sub_cnt + 1,
                                  leftOfpost, i);
            opnv->rc = __build_pp(leftOfpre + sub_cnt + 2, rightOfpre, i + 1,
                                  rightOfpost - 1);
        } else {
            opnv->lc = __build_pp(leftOfpre + 1, rightOfpre, leftOfpost,
                                  rightOfpost - 1);
    ...
    int __leafcount() {
        ...
        while (q.size()) {
            ...
            if (!__tmp->lc && !__tmp->rc) cnt++;
            if (__tmp->lc) q.push_back(__tmp->lc);
            if (__tmp->rc) q.push_back(__tmp->rc);
        }
        return cnt;
    }
    ...
    void __del_targetSub(binode_ptr<_Tp> opnv, int target) {
        ...
        while (q.size()) {
            ...
            if (x->lc) q.push_back(x->lc);
            if (x->rc) q.push_back(x->rc);
        }
    }
    ...
    bool __isomprphic(binode_ptr<_Tp> v1, binode_ptr<_Tp> v2) {
        ...
        if (!v1->lc && !v2->lc) return __isomprphic(v1->rc, v2->rc);
    ...
    bool __TreeSimilar(binode_ptr<_Tp> v1, binode_ptr<_Tp> v2) {
        if (!v1 && !v2) return 1;
        if (!v1 || !v2)  // if wanna test the two trees is identical , plus "||
                         // v1->val != v2->val"
            return 0;
    ...
    double __infix_val(binode_ptr<string> opnv) {
        if (!opnv) return 0;
        if (opnv->lc && opnv->rc) {
            double a = stof(__infix_val(opnv->lc)),
                   b = stof(__infix_val(opnv->rc));
    ...
    int __diameter() {
        ...
        binode_ptr<_Tp> __tmp;
        while (q.size()) {
            if (q.size() > 1) last = level;
            while (q.size()) {
                __tmp = q.front(), q.pop_front();
                if (__tmp->lc) nexq.push_back(__tmp->lc);
                if (__tmp->rc) nexq.push_back(__tmp->rc);
            }
            level++;
            swap(q, nexq);
        }
   ...
   public:
    ...
    inline void clear() {
        ...
        for (auto &ptr : __memory_pool) delete ptr;
        __memory_pool.clear();
    }
    ...
    void printhorizon() {
        ...
        _unitsize =
            IS_CHAR<_Tp>() ? 1 : to_string(_root->rightest()->val).size();
        if (_isRBtree) _unitsize++;
        _direct2l = _direct2r = string(_unitsize, ' ');
        _direct2l[_unitsize / 2] = '/';
        _direct2r[_unitsize / 2] = '\\';
        this->__display_buffer = vector<vector<string>>(
            40, vector<string>(MAXCOL, string(_unitsize, ' ')));
        __print_horizon(_root, 0, pow(2, _root->height) - 1,
                        pow(2, _root->height - 1));
        int n = _root->height * 2 + 1, i, j,
            breadth = pow(2, _root->height + 1) + 1;
        for (i = 0; i < n; ++i) {
            for (j = 0; j < breadth; ++j) cout << this->__display_buffer[i][j];
            cout << endl;
        }
        printf("🌲\n");
        if (_ishuffman)
    ...
    double infixval() { return __infix_val(_root); }
    bool iscmp() {
        if (!_root) return true;
        deque<binode_ptr<_Tp>> q;
        q.push_back(_root);
    ...
    void generate_preorder() {
        ...
        while (s.size()) {
            ...
            if (opnv->rc) s.push(opnv->rc);
            if (opnv->lc) s.push(opnv->lc);
        }
    ...
    void generate_postorder() {
        ...
        while (s.size()) {
            ...
            if (isMyTurn)
                ...
                if (opnv->rc) s.push(make_pair(opnv->rc, false));
                if (opnv->lc) s.push(make_pair(opnv->lc, false));
            }
        }
        this->postorder = resSeq;
    }
    binode_ptr<_Tp> get_lca(binode_ptr<_Tp> p, binode_ptr<_Tp> q) {
        while (p->height < q->height) p = p->parent;
        while (q->height < p->height) q = q->parent;
    ...
    bool erase(const _Tp &val) { return false; }
    bool count(const _Tp &val) {
        int c = 0;
        intrav([&](binode_ptr<_Tp> v, int c) {
            if (v->val == val) c++;
        });
        return c;
    }

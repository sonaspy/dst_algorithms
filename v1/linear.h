...
struct listNode {
    ...
    listNode(_Tp v, lnode_ptr<_Tp> p = nullptr, lnode_ptr<_Tp> nex = nullptr)
        : val(v), freq(0), next(nex), pre(p) {}
};
...
#define __insert_head(dum, __1st) \
    ...
    inline lnode_ptr<_Tp> __newlnode(const _Tp &val, lnode_ptr<_Tp> p = nullptr,
                                     lnode_ptr<_Tp> nex = nullptr) {
    ...
    void __merge_list(
        lnode_ptr<_Tp> l2) {  // 2 increasing order list merge to a new
                              // increasing order list. in-place (rear insert)
        lnode_ptr<_Tp> p = _dummy->next, q = l2->next, r;
        _dummy->next = nullptr;
        delete l2;
        r = _dummy;
        while (p && q) {
            p->val <= q->val ? r->next = p, p = p->next : r->next = q,
                               q = q->next;
            r = r->next;
        }
        r->next = p ? p : q;
    ...
    void __merge_list2(
        lnode_ptr<_Tp> l2) {  // 2 increasing order list merge to a new
                              // decreasing order list. in-place (head insert)
    ...
    void __add_pre() {
        ...
        for (; _walk; _walk = _walk->next) _walk->pre = p, p = _walk;
    }
   ...
   public:
    ...
    void clear() {
        _dummy = nullptr;
        for (auto &ptr : __memoflnode) delete ptr;
        __memoflnode.clear();
        _size = 0;
    }
    void reverse() {
    ...
    void erase_val(const _Tp &val) { __erase1(val); }
    void erase(int _pos) {
        if (_pos < _size) {
    ...
    void insert(int _pos, const _Tp &val) {
        if (_pos <= _size) {
            ...
            for (; _pos--; _walk = _walk->next) p = _walk;
    ...
    void insertsort() {
        ...
        while (_walk) {
            the_nex = _walk->next;
            for (pre = _dummy; pre->next && pre->next->val <= _walk->val;
                 pre = pre->next)
                ;
            _walk->next = pre->next;
            pre->next = _walk;
    ...
    void locate(const _Tp &x) {
        ...
        while (left != _dummy && left->freq <= p->freq) left = left->pre;
    ...
    bool is_sorted() {
        if (!_dummy->next || !_dummy->next->next) return true;
        lnode_ptr<_Tp> p = _dummy->next, _walk = p->next;
        while (_walk) {
            if (_walk->val > p->val) return false;
            p = _walk;
            _walk = _walk->next;
        }
        return true;
    ...
    lnode_ptr<_Tp> find_common_p(lnode_ptr<_Tp> &l1, lnode_ptr<_Tp> &l2) {
        ...
        step = abs(len1 - len2);
        while (step--) thelong = thelong->next;
    ...
    void pop_front() {
        if (_size) {
            ...
            if (_dummy->next) _dummy->next->pre = _dummy;
            __release(tmp);
        }
    }
    void pop_back() {
    ...
    _Tp back() { return _rear->val; }
    _Tp front() { return _dummy->next->val; }
};
...
class tag_queue {
   ...
   public:
    ...
    inline bool isempty() { return front == rear && !tag; }
    inline bool isfull() { return front == rear && tag; }
    bool enqueue(int x) {
        if (isfull()) return 0;
        data[rear] = x;
        rear = (rear + 1) % maxsize;
...
class share_stack {
   ...
   public:
    ...
    inline bool isempty(int id) {
        return id == 1 ? top1 == -1 : top2 == maxsize;
    }
...
class _stack {
   ...
   public:
    _stack() {}
    inline void empty() { return data.empty(); }
    inline void push(const _Tp &val) { data.push_back(val); }
    inline _Tp top() { return data.back(); }
...
class _deque {
   ...
   public:
    ...
    int size() { return (_rear - _front + _maxsize) % _maxsize; }
    void resize(int n) {
        data.resize(n + 1);
        _maxsize = n + 1;
    }
    void clear() { _rear = _front = 0; }
    bool empty() { return _rear == _front; }
    bool full() { return (_rear + 1) % _maxsize == _front; }
    void pop_back() {
        if (empty()) return;
    ...
    _Tp front() { return data[(_front + 1) % _maxsize]; }
    _Tp back() { return data[_rear]; }
};
...
class NQueen {
   public:
    vector<vector<string>> solveNQueens(int n) {
        this->columns =
            vector<bool>(n, false);  // if there is a queen in this column.
        this->main_diag =
            vector<bool>(2 * n - 1, false);  // if there is a queen in this diag
        this->vice_diag =
            vector<bool>(2 * n - 1, false);  // if there is a queen in this diag
    ...
    void dfs(vector<vector<string>> &res, int row) {
        ...
        for (int j = 0; j < n; j++) {
            if (!columns[j] && !main_diag[row - j + n - 1] &&
                !vice_diag[row + j]) {
                C[row] = j;
                columns[j] = main_diag[row - j + n - 1] = vice_diag[row + j] =
                    true;
                dfs(res, row + 1);
                C[row] = -1;
                columns[j] = main_diag[row - j + n - 1] = vice_diag[row + j] =
...
4.运算符 I.if pri(this) > pri(top),push. else, pop output, then compare again,
until push. 5.'(' push之前pri最高， push后pri最低 6.表达式处理完毕后 output rest
in stack (before start, put a guard in stack ,pri = -1(lowest) );
*/
};  // namespace dsa

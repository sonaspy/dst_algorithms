...
template <class K, class E>
int hashTable<K, E>::search(K &theKey) {  // Search an open addressed hash table
                                          // for a pair with key theKey.
...
pair<K, E> *hashTable<K, E>::find(
    ...
    if (table[b] == NULL || table[b]->first != theKey) return NULL;  // no match

    return table[b];  // matching pair
}

template <class K, class E>
void hashTable<K, E>::insert(
    pair<K, E>
    ...
    if (table[b] == NULL) {
        ...
        if (table[b]->first ==
            thePair.first) {  // duplicate, change table[b]->second
...
void hashTable<K, E>::output(
    ostream &out) {  // Insert the hash table into the stream out.

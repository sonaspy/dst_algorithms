...
class __Union {
   protected:
    inline int __getRoot(int id) {
        return __union_arr[id] < 0
                   ? id
                   : __union_arr[id] = __getRoot(__union_arr[id]);
    }

   public:
    vector<int> __union_arr;
    __Union(int n = 1000) { __union_arr = vector<int>(n, -1); }
    void clear() { fill(__union_arr.begin(), __union_arr.end(), -1); }

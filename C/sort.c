...
void PercDown(DataType A[], int i, int N)
{
    int Child;
    DataType Tmp;
...
void Merge(DataType A[], DataType TmpArray[],
...
/* START: fig7_9.txt */
void MSort(DataType A[], DataType TmpArray[],
           int Left, int Right)
{
    int Center;
    if (Left < Right)
...
void Qselect(DataType A[], int k, int Left, int Right)
{
    int i, j;
    DataType Pivot;
    /* 1*/ if (Left + Cutoff <= Right)
...
void Checksort(DataType A[], int N)
{
    int i;
    for (i = 0; i < N; i++)
        if (A[i] != i)
...
void Copy(DataType Lhs[], const DataType Rhs[], int N)
{
    int i;
    for (i = 0; i < N; i++)
        Lhs[i] = Rhs[i];

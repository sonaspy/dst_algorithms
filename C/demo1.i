...
int main()
{
    ...
    stu.id = 9527;
    stu.age = 24;
    __builtin___strcpy_chk (stu.name, "zhouxingxing", __builtin_object_size (stu.name, 2 > 1 ? 1 : 0));

    pstu = ({ const typeof( ((struct student *)0)->id ) *__mptr = (&stu.id); (struct student *)( (char *)__mptr - ((size_t) & ((struct student *)0)->id) ); });

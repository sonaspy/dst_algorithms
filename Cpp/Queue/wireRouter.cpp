...
// global variables
int **grid;     // 2D array
int size_m;       // number of rows and columns in the grid
...
void inputData()
{
    ...
    make2dArray(grid, size_m + 2, size_m + 2);
...
bool findPath()
{
    ...
    for (int i = 0; i <= size_m + 1; i++)
        {
            grid[0][i] = grid[size_m + 1][i] = 1; // bottom and top
            grid[i][0] = grid[i][size_m + 1] = 1; // left and right
        }
    position here = start;

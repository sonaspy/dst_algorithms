...
{
    // Input the maze.
    cout << "Enter maze size_m" << endl;
    cin >> size_m;
    make2dArray(maze, size_m + 2, size_m + 2);
...
bool findPath()
{
    // Find a path from (1,1) to the exit (size_m, size_m).
    ...
    for (int i = 0; i <= size_m + 1; i++)
        {
            maze[0][i] = maze[size_m + 1][i] = 1; // bottom and top
            maze[i][0] = maze[i][size_m + 1] = 1; // left and right
        }
    position here;
    here.row = 1;
    here.col = 1;
    ...
    int lastOption = 3;
    // search for a path
    while (here.row != size_m || here.col != size_m)
        {
            // not exit

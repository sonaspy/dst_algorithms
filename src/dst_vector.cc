...
class __vector_alloc_base {
  public:
    typedef
        typename __alloc_traits<_Tp, _Allocator>::allocator_type allocator_type;
...
class __vector_alloc_base<_Tp, _Allocator, true> {
  public:
    ...
    __vector_alloc_base(const allocator_type &)
...
struct __vector_base
    : public __vector_alloc_base<_Tp, _Alloc,
                                 __alloc_traits<_Tp, _Alloc>::_S_instanceless> {
    typedef __vector_alloc_base<_Tp, _Alloc,
                                __alloc_traits<_Tp, _Alloc>::_S_instanceless>
        __base;
...
class vector : public __vector_base<_Tp, _Alloc> {
  ...
  public:
    ...
    inline size_type max_size() const { return size_type(-1) / sizeof(_Tp); }
    ...
    vector(const vector<_Tp, _Alloc> &__x)
        : __base(__x.size(), __x.get_allocator()) {
...
operator=(const vector<_Tp, _Alloc> &__x) {
    if (&__x != this) {
        ...
        if (__xlen > capacity()) {
            iterator __tmp =
                _M_allocate_and_copy(__xlen, __x.begin(), __x.end());
            destroy(_M_start, _M_finish);
            _M_deallocate(_M_start, _M_end_of_storage - _M_start);
            _M_start = __tmp;
...
void vector<_Tp, _Alloc>::_M_assign_aux(_ForwardIter __first,
                                        _ForwardIter __last,
                                        forward_iterator_tag) {
    size_type __len = 0;
...
void vector<_Tp, _Alloc>::_M_insert_aux(iterator __position, const _Tp &__x) {
    if (_M_finish != _M_end_of_storage) // storage still suffice the demand
    {
        ...
        *__position = __x_copy;
        ...
        __DST_TRY {
            __new_finish =
                uninitialized_copy(_M_start, __position, __new_start);
        ...
        if (size_type(_M_end_of_storage - _M_finish) >= __n) {
            ...
            __DST_TRY {
                __new_finish =
                    uninitialized_copy(_M_start, __position, __new_start);
                __new_finish =
                    uninitialized_copy(__first, __last, __new_finish);
                __new_finish =
                    uninitialized_copy(__position, _M_finish, __new_finish);
            }
            __DST_UNWIND((destroy(__new_start, __new_finish),

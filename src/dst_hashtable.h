...
private:
    ...
    void _M_put_node(__node *__p) { _M_node_allocator_type::deallocate(__p, 1); }
...
public:
    ...
    size_type elems_in_bucket(size_type __bucket) const
    {
        ...
        for (__node *__cur = _M_buckets[__bucket]; __cur; __cur = __cur->_M_next)
            __result += 1;
        return __result;
    }
    ...
    size_type count(const key_type &__key) const
    {
        ...
        for (const __node *__cur = _M_buckets[__n]; __cur; __cur = __cur->_M_next)
...
hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::equal_range(const key_type &__key)
{
    ...
    for (__node *__first = _M_buckets[__n]; __first; __first = __first->_M_next)
        ...
        {
            for (__node *__cur = __first->_M_next; __cur; __cur = __cur->_M_next)
                if (!_M_equals(_M_get_key(__cur->_M_val), __key))
...
hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::equal_range(const key_type &__key) const
{
    ...
    for (const __node *__first = _M_buckets[__n];
         __first;
         __first = __first->_M_next)
    {
        if (_M_equals(_M_get_key(__first->_M_val), __key))
        {
            for (const __node *__cur = __first->_M_next;
                 __cur;
                 __cur = __cur->_M_next)
                if (!_M_equals(_M_get_key(__cur->_M_val), __key))
...
template <class _Val, class _Key, class _HF, class _Ex, class _Eq, class _All>
void hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::_M_erase_bucket(const size_type __n, __node *__first, __node *__last)
{
    __node *__cur = _M_buckets[__n];
    if (__cur == __first)
...
template <class _Val, class _Key, class _HF, class _Ex, class _Eq, class _All>
void hashtable<_Val, _Key, _HF, _Ex, _Eq, _All>::_M_erase_bucket(const size_type __n, __node *__last)
{
    __node *__cur = _M_buckets[__n];
    while (__cur != __last)
    {
    ...
    __DST_TRY
    {
        for (size_type __i = 0; __i < __ht._M_buckets.size(); ++__i)
        {
            ...
            if (__cur)
            {
                ...
                for (__node *__next = __cur->_M_next;
                     __next;
                     __cur = __next, __next = __cur->_M_next)
                {

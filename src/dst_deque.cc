...
struct __deque_iterator {
    ...
    __deque_iterator(_Tp *__x, __map_pointer __y)
        : _M_cur(__x), _M_first(*__y), _M_last(*__y + _S_buffer_size()),
          __map_node(__y) {}

    __deque_iterator() : _M_cur(0), _M_first(0), _M_last(0), __map_node(0) {}

    __deque_iterator(const iterator &__x)
        : _M_cur(__x._M_cur), _M_first(__x._M_first), _M_last(__x._M_last),
          __map_node(__x.__map_node) {}
    ...
    __self &operator+=(difference_type __n) {
        difference_type __offset = _M_cur - _M_first + __n;
        if (0 <= __offset && __offset < difference_type(_S_buffer_size()))
            _M_cur += __n;
        else {
            ...
            _M_set_node(__map_node + __node_offset);
            _M_cur =
                _M_first +
    ...
    bool operator==(const __self &__x) const { return _M_cur == __x._M_cur; }
    bool operator!=(const __self &__x) const { return !(*this == __x); }
    bool operator<(const __self &__x) const {
        return (__map_node == __x.__map_node) ? (_M_cur < __x._M_cur)
                                              : (__map_node < __x.__map_node);
    }
    ...
    void _M_set_node(__map_pointer __new_node) {
...
template <class _Tp, class _Ref, class _Ptr>
inline _Tp *value_type(const __deque_iterator<_Tp, _Ref, _Ptr> &) {
    return 0;
}
...
class __deque_alloc_base {
  public:
    ...
    __deque_alloc_base(const allocator_type &__a)
        : _M_node_allocator(__a), _M_map_allocator(__a), _M_map(0),
          _M_map_size(0) {}

  protected:
    typedef typename _Alloc_traits<_Tp *, _Alloc>::allocator_type
        __map_allocator_type;

    allocator_type _M_node_allocator;
    __map_allocator_type _M_map_allocator;
...
class __deque_alloc_base<_Tp, _Alloc, true> {
  public:
    ...
    __deque_alloc_base(const allocator_type &) : _M_map(0), _M_map_size(0) {}
...
template <class _Tp, class _Alloc>
class __deque_base
    : public __deque_alloc_base<_Tp, _Alloc,
                                _Alloc_traits<_Tp, _Alloc>::_S_instanceless> {
  public:
    typedef __deque_alloc_base<_Tp, _Alloc,
                               _Alloc_traits<_Tp, _Alloc>::_S_instanceless>
        __base;
    ...
    __deque_base(const allocator_type &__a, size_t __num_elements)
        : __base(__a), _M_start(), _M_finish() {
        _M_initialize_map(__num_elements);
    }
    __deque_base(const allocator_type &__a)
        : __base(__a), _M_start(), _M_finish() {}
    ~__deque_base();
...
class __deque_base {
  public:
    ...
    __deque_base(const allocator_type &, size_t __num_elements)
        : _M_map(0), _M_map_size(0), _M_start(), _M_finish() {
        _M_initialize_map(__num_elements);
    }
    __deque_base(const allocator_type &)
        : _M_map(0), _M_map_size(0), _M_start(), _M_finish() {}
    ~__deque_base();
...
template <class _Tp, class _Alloc>
void __deque_base<_Tp, _Alloc>::_M_initialize_map(size_t __num_elements) {
    ...
    __DST_TRY { _M_create_nodes(__nstart, __nfinish); }
    __DST_UNWIND(
        (_M_deallocate_map(_M_map, _M_map_size), _M_map = 0, _M_map_size = 0));
    _M_start._M_set_node(__nstart);
    _M_finish._M_set_node(__nfinish - 1);
    _M_start._M_cur = _M_start._M_first;
    _M_finish._M_cur =
        _M_finish._M_first + __num_elements % __deque_buf_size(sizeof(_Tp));
}

template <class _Tp, class _Alloc>
void __deque_base<_Tp, _Alloc>::_M_create_nodes(_Tp **__nstart,
                                                _Tp **__nfinish) {
    _Tp **__cur;
    __DST_TRY {
    ...
    __DST_UNWIND(_M_destroy_nodes(__nstart, __cur));
}
...
class deque : protected __deque_base<_Tp, _Alloc> {
  ...
  protected:
    ...
    using __base::_M_allocate_map;
    using __base::_M_allocate_node;
    using __base::_M_create_nodes;
    using __base::_M_deallocate_map;
    using __base::_M_deallocate_node;
    using __base::_M_destroy_nodes;
    using __base::_M_finish;
    using __base::_M_initialize_map;
    using __base::_M_map;
    using __base::_M_map_size;
  ...
  public: // Constructor, destructor.
    ...
    void swap(deque &__x) {
        ...
        __VDSA::swap(_M_map, __x._M_map);
        __VDSA::swap(_M_map_size, __x._M_map_size);
    }
  ...
  protected: // Internal insert functions
    ...
    template <class _ForwardIterator>
    void insert(iterator __pos, _ForwardIterator __first,
    ...
    void _M_insert_aux(iterator __pos, const value_type *__first,
                       const value_type *__last, size_type __n);

    void _M_insert_aux(iterator __pos, const_iterator __first,
  ...
  protected: // Allocation of _M_map and nodes
    ...
    void _M_reserve_map_at_back(size_type __nodes_to_add = 1) {
        if (__nodes_to_add + 1 > _M_map_size - (_M_finish.__map_node - _M_map))
            _M_reallocate_map(__nodes_to_add, false);
    }

    void _M_reserve_map_at_front(size_type __nodes_to_add = 1) {
        if (__nodes_to_add > size_type(_M_start.__map_node - _M_map))
...
template <class _Tp, class _Alloc>
template <class _InputIter>
void deque<_Tp, _Alloc>::_M_assign_aux(_InputIter __first, _InputIter __last,
                                       input_iterator_tag) {
...
void deque<_Tp, _Alloc>::_M_fill_insert(iterator __pos, size_type __n,
    ...
    if (__pos._M_cur == _M_start._M_cur) {
        ...
        __DST_UNWIND(
            _M_destroy_nodes(__new_start.__map_node, _M_start.__map_node));
        ...
        __DST_UNWIND(_M_destroy_nodes(_M_finish.__map_node + 1,
                                      __new_finish.__map_node + 1));
    } else
...
template <class _Tp, class _Alloc>
void deque<_Tp, _Alloc>::insert(iterator __pos, const value_type *__first,
                                const value_type *__last) {
...
template <class _Tp, class _Alloc>
void deque<_Tp, _Alloc>::insert(iterator __pos, const_iterator __first,
...
deque<_Tp, _Alloc>::erase(iterator __first, iterator __last) {
    if (__first == _M_start && __last == _M_finish) {
        ...
        if (__elems_before < difference_type((this->size() - __n) / 2)) {
            ...
            _M_destroy_nodes(__new_finish.__map_node + 1,
                             _M_finish.__map_node + 1);
            _M_finish = __new_finish;
        }
...
void deque<_Tp, _Alloc>::clear() {
    ...
    if (_M_start.__map_node != _M_finish.__map_node) {
...
void deque<_Tp, _Alloc>::_M_fill_initialize(const value_type &__value) {
    __map_pointer __cur;
    __DST_TRY {
        for (__cur = _M_start.__map_node; __cur < _M_finish.__map_node;
             ++__cur) {
            uninitialized_fill(*__cur, *__cur + _S_buffer_size(), __value);
        }
        uninitialized_fill(_M_finish._M_first, _M_finish._M_cur, __value);
    }
    __DST_UNWIND(destroy(_M_start, iterator(*__cur, __cur)));
}
...
void deque<_Tp, _Alloc>::_M_range_initialize(_ForwardIterator __first,
    ...
    __DST_TRY {
        for (__cur_node = _M_start.__map_node;
             __cur_node < _M_finish.__map_node; ++__cur_node) {
            _ForwardIterator __mid = __first;
    ...
    __DST_UNWIND(destroy(_M_start, iterator(*__cur_node, __cur_node)));
}
...
void deque<_Tp, _Alloc>::_M_push_back_aux(const value_type &__t) {
    ...
    *(_M_finish.__map_node + 1) = _M_allocate_node();
    ...
    __DST_UNWIND(_M_deallocate_node(*(_M_finish.__map_node + 1)));
}
...
void deque<_Tp, _Alloc>::_M_push_front_aux(const value_type &__t) {
    ...
    *(_M_start.__map_node - 1) = _M_allocate_node();
    __DST_TRY {
        _M_start._M_set_node(_M_start.__map_node - 1);
        _M_start._M_cur = _M_start._M_last - 1;
        construct(_M_start._M_cur, __t_copy);
    }
    __DST_UNWIND((++_M_start, _M_deallocate_node(*(_M_start.__map_node - 1))));
}
...
template <class _Tp, class _Alloc>
template <class _InputIterator>
void deque<_Tp, _Alloc>::insert(iterator __pos, _InputIterator __first,
...
template <class _Tp, class _Alloc>
template <class _ForwardIterator>
void deque<_Tp, _Alloc>::insert(iterator __pos, _ForwardIterator __first,
                                _ForwardIterator __last, forward_iterator_tag) {
    size_type __n = 0;
    distance(__first, __last, __n);
    if (__pos._M_cur == _M_start._M_cur) {
    ...
    if (__elems_before < difference_type(__length / 2)) {
        ...
        __DST_TRY {
            if (__elems_after > difference_type(__n)) {
                ...
                __uninitialized_fill_copy(_M_finish,
...
template <class _Tp, class _Alloc>
template <class _ForwardIterator>
void deque<_Tp, _Alloc>::_M_insert_aux(iterator __pos, _ForwardIterator __first,
                                       _ForwardIterator __last, size_type __n) {
    ...
    if (__elemsbefore < __length / 2) {
        ...
        __DST_TRY {
            if (__elemsafter > difference_type(__n)) {
                ...
                __uninitialized_copy_copy(__mid, __last, __pos, _M_finish,
                                          _M_finish);
                _M_finish = __new_finish;
...
template <class _Tp, class _Alloc>
void deque<_Tp, _Alloc>::_M_insert_aux(iterator __pos, const_iterator __first,
                                       const_iterator __last, size_type __n) {
    const difference_type __elemsbefore = __pos - _M_start;
    size_type __length = size();
    if (__elemsbefore < __length / 2) {
...
void deque<_Tp, _Alloc>::_M_new_elements_at_front(size_type __new_elems) {
    ...
    __DST_TRY {
        for (__i = 1; __i <= __new_nodes; ++__i)
            *(_M_start.__map_node - __i) = _M_allocate_node();
    }
#ifdef __DST_USE_EXCEPTIONS
    catch (...) {
        for (size_type __j = 1; __j < __i; ++__j)
            _M_deallocate_node(*(_M_start.__map_node - __j));
        throw;
    }
...
void deque<_Tp, _Alloc>::_M_new_elements_at_back(size_type __new_elems) {
    ...
    __DST_TRY {
        for (__i = 1; __i <= __new_nodes; ++__i)
            *(_M_finish.__map_node + __i) = _M_allocate_node();
    }
#ifdef __DST_USE_EXCEPTIONS
    catch (...) {
        for (size_type __j = 1; __j < __i; ++__j)
            _M_deallocate_node(*(_M_finish.__map_node + __j));
        throw;
    }
#endif /* __DST_USE_EXCEPTIONS */
}
...
void deque<_Tp, _Alloc>::_M_reallocate_map(size_type __nodes_to_add,
                                           bool __add_at_front) {
    size_type __old_num_nodes = _M_finish.__map_node - _M_start.__map_node + 1;
    ...
    if (_M_map_size > 2 * __new_num_nodes) {
        ...
        if (__new_nstart < _M_start.__map_node)
            copy(_M_start.__map_node, _M_finish.__map_node + 1, __new_nstart);
        else
            copy_backward(_M_start.__map_node, _M_finish.__map_node + 1,
                          __new_nstart + __old_num_nodes);
    } else {
        size_type __new_map_size =
            _M_map_size + max(_M_map_size, __nodes_to_add) + 2;

        __map_pointer __new_map = _M_allocate_map(__new_map_size);

...
class queue {
    ...
    friend bool __DSA_QUALIFIER operator==
        __DST_NULL_TMPL_ARGS(const queue &, const queue &);
    friend bool __DSA_QUALIFIER operator<
        __DST_NULL_TMPL_ARGS(const queue &, const queue &);
...
template <class _Tp, class _Sequence __DST_DEPENDENT_DEFAULT_TMPL(vector<_Tp>),
          class _Compare __DST_DEPENDENT_DEFAULT_TMPL(

...
inline _Rb_tree_node_base *_Rb_tree_rebalance_for_erase(
    ...
    else if (__y->_M_right ==
             nullptr)        // __z has exactly one non-null child. y == z.
    ...
    if (__y->_M_color != _S_rb_tree_red) {
        while (__x != __root &&
               (__x == nullptr || __x->_M_color == _S_rb_tree_black))
...
class _Rb_tree : protected _Rb_tree_base<_Value, _Alloc> {
  ...
  public:
    ...
    bool empty() const { return _M_node_count == nullptr; }
...
_Rb_tree<_Key, _Val, _KeyOfValue, _Compare, _Alloc>::insert_unique(
    ...
    if (__position._M_node == _M_header->_M_left) { // begin()
        ...
        if (_M_key_compare(_S_key(__before._M_node), _KeyOfValue()(__v)) &&
            _M_key_compare(_KeyOfValue()(__v), _S_key(__position._M_node))) {
            if (_S_right(__before._M_node) == nullptr)
...
template <class _II>
void _Rb_tree<_Key, _Val, _KoV, _Cmp, _Alloc>::insert_equal(_II __first,
                                                            _II __last) {
...
template <class _Key, class _Val, class _KoV, class _Cmp, class _Alloc>
template <class _II>
void _Rb_tree<_Key, _Val, _KoV, _Cmp, _Alloc>::insert_unique(_II __first,
                                                             _II __last) {
    for (; __first != __last; ++__first)
...
inline pair<
    ...
    typename _Rb_tree<_Key, _Value, _KoV, _Compare, _Alloc>::const_iterator>
_Rb_tree<_Key, _Value, _KoV, _Compare, _Alloc>::equal_range(
    const _Key &__k) const {
...
template <class _Key, class _Value, class _KeyOfValue, class _Compare,
          class _Alloc>
bool _Rb_tree<_Key, _Value, _KeyOfValue, _Compare, _Alloc>::__rb_verify()
    const {
    if (_M_node_count == nullptr || begin() == end())
        return _M_node_count == nullptr && begin() == end() &&

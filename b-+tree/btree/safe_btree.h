...
class safe_btree_iterator {
  ...
  public:
    ...
    safe_btree_iterator(const iterator &x)
        : generation_(x.generation()), key_(x.key()), iter_(x.iter()),
          tree_(x.tree()) {}
    ...
    Iterator *mutable_iter() const {
        if (generation_ != tree_->generation()) {
            if (generation_ > 0) {
                // This does the wrong thing for a multi{set,map}. If my iter
                // was pointing to the 2nd of 2 values with the same key, then
                // this will reset it to point to the first. This is why we
                // don't provide a safe_btree_multi{set,map}.
...
class safe_btree {
  ...
  public:
    ...
    typedef safe_btree_iterator<const self_type, tree_const_iterator>
        const_iterator;

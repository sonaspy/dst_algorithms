...
struct remove_const<pair<T, U>> {
    typedef pair<typename remove_const<T>::type, typename remove_const<U>::type>
        type;
};
...
class base_checker {
  ...
  public:
    ...
    IterType iter_check(IterType tree_iter,
                        CheckerIterType checker_iter) const {
    ...
    template <typename IterType, typename CheckerIterType>
    IterType riter_check(IterType tree_iter,
                         CheckerIterType checker_iter) const {
    ...
    equal_range(const key_type &key) const {
        std::pair<typename CheckerType::const_iterator,
            ...
            checker_res = checker_.equal_range(key);
        std::pair<const_iterator, const_iterator> tree_res =
    ...
    void erase(iterator begin, iterator end) {
        ...
        typename CheckerType::iterator checker_begin =
            checker_.find(begin.key());
...
void DoTest(const char *name, T *b, const std::vector<V> &values) {
    ...
    for (int i = 0; i < values.size(); i++) {
        mutable_b.insert(mutable_b.upper_bound(key_of_value(values[i])),
                         values[i]);
    }
...
class TestAllocator : public Alloc {
  public:
    ...
    template <class U>
    struct rebind {
        typedef TestAllocator<U, typename Alloc::template rebind<U>::other>
            other;
    };

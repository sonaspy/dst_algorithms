...
template <class T>
T __select(T a[], int leftEnd, int rightEnd, int k)
{
    ...
    if (rightCursor - leftEnd + 1 < k)
        return __select(a, rightCursor + 1, rightEnd,
                        k - rightCursor + leftEnd - 1);
    else
        return __select(a, leftEnd, rightCursor - 1, k);
}
